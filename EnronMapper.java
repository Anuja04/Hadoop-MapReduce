   	...
   	public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
        ...
        if(line.startsWith("X-To:")) {
            ...
            while(tokens.hasMoreTokens()) {
                ...
                String tempVal = tokens.nextToken();
                if((tempVal.contains("@")) && (tempVal.contains(".")))
                {
                    String[] split2 = tempVal.split("@");
                    String[] split1 = split2[1].split("\\s+");
                    split1[0].replaceAll(">", "");
                    split1[0].replaceAll(",","");
                    split1[0].replaceAll("'","");
                    domain = split1[0].replaceAll("[^A-Za-z0-9. ]","");
